...
「要素に配列・マップを含まない」というルールの唯一の例外が `<<...>>`
(CBOR Sequence Literal / embedded CBOR) です。`<<...>>` はネスト構造を
表示するものではなく、エンコードされたアイテムのフラットな並びなので、
要素自体が配列・マップであっても、その要素自身のレンダリング結果が
1 行に収まる限り、そのままインライン化されます。
