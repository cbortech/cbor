/**
 * `Sample.showsCddl` behavior: unlike `requiresCddl` (which always forces
 ...
 * "selecting a sample that requires CDDL..." tests), `showsCddl` only opens
 ...
 * moment the sample is selected, which no other test file's shared session
...
describe('a showsCddl sample, selected before the reader has touched the CDDL toggle', () => {
  beforeAll(async () => {
  ...
  test('opens the pane, without persisting it to ?cddl=', () => {
    ...
    const sample = SAMPLES.find((s) => s.showsCddl);
    expect(sample).toBeDefined();
    byId<HTMLSelectElement>('samples').value = sample!.name;
  ...
  test("a reader's own explicit close is respected on every later showsCddl sample", () => {
    ...
    // Browse elsewhere, then back to the same showsCddl sample.
